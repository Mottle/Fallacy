...
class HealthHud : HudLayerImpl("health") {
    ...
    companion object {
        ...
        internal fun calculateScaledColor(d1: Float, d2: Float, effect: HealthEffect): RGB {
            val (colors, fractions) = when (effect) {
                HealthEffect.NONE -> Pair(NORMAL_COLORS, listOf(.25f, .5f, .75f))
                HealthEffect.POISON -> Pair(POISON_COLORS, listOf(.25f, .5f, .75f))
                HealthEffect.WITHER -> Pair(WITHER_COLORS, listOf(.25f, .5f, .75f))
            }
        ...
        fun getHealthEffect(player: Player): HealthEffect {
            if (player.hasEffect(MobEffects.POISON)) return HealthEffect.POISON
            if (player.hasEffect(MobEffects.WITHER)) return HealthEffect.WITHER
            return HealthEffect.NONE
        }
    }
    ...
    override fun renderIcon(
        ...
        val icon = if (isHardcore) when (effect) {
            HealthEffect.NONE -> HARDCORE_ICON
            HealthEffect.POISON -> HARDCORE_POISON_ICON
            HealthEffect.WITHER -> HARDCORE_WITHER_ICON
        } else when (effect) {
            HealthEffect.NONE -> NORMAL_ICON
            HealthEffect.POISON -> POISON_ICON
            HealthEffect.WITHER -> WITHER_ICON
        }
